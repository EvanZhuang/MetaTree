...
def tree_eval_process(data, num_classes=2):
    data["input_x"] = data["X"]
    data['input_y'] = torch.nn.functional.one_hot(torch.tensor(data["y"]), num_classes=num_classes).float()
...
def main():
    ...
    if args.dataset_name is not None:
        if args.load_from_local:
            ...
            raw_datasets = load_dataset(args.dataset_name, args.dataset_config_name, num_proc=os.cpu_count()-1)
    ...
    train_dataloader = DataLoader(
        train_dataset, shuffle=True, collate_fn=default_data_collator, batch_size=args.per_device_train_batch_size, num_workers=os.cpu_count()-1
    )
    eval_dataloader = DataLoader(
        eval_dataset, shuffle=True, collate_fn=default_data_collator, batch_size=args.per_device_eval_batch_size, num_workers=os.cpu_count()-1
    )
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers("treegen_stats_metatree", experiment_config)
    ...
    while len(decision_tree_forest.trees) < args.max_train_steps:
        for step, batch in enumerate(train_dataloader):
            if args.deepspeed:
                for key in batch.keys():
                    if batch[key].dtype == torch.float32: batch[key] = batch[key].to(dtype=torch.float16)
            with torch.no_grad():
                outputs = model.generate_decision_tree(**batch, depth=args.depth, deterministic=deterministic)
                pred = outputs.logits.argmax(dim=-1)
                x = batch['input_x']
                y = batch['input_y']
                decision_tree_forest.add_tree(DecisionTree(auto_dims=outputs.metatree_dimensions, auto_thresholds=outputs.tentative_splits, input_x=x, input_y=y, depth=args.depth))
                tree_pred = decision_tree_forest.trees[-1].predict(batch["input_x"])
            if len(decision_tree_forest.trees) >= (args.max_train_steps-1):
                # We have built enough number of Trees
                break
    # Train Eval
    for step, batch in enumerate(train_dataloader):
        with torch.no_grad():
            tree_pred = decision_tree_forest(batch["input_x"], hard=False)
            tree_pred = tree_pred.argmax(dim=-1)
            metric.add_batch(predictions=tree_pred.view(-1), references=batch["input_y"].argmax(dim=-1).view(-1))
    
    for step, batch in enumerate(eval_dataloader):
        with torch.no_grad():
            tree_pred = decision_tree_forest(batch["input_x"], hard=False)
            tree_pred = tree_pred.argmax(dim=-1)
            metatree_eval_metric.add_batch(predictions=tree_pred.view(-1), references=batch["input_y"].argmax(dim=-1).view(-1))
    train_output = metric.compute()
    eval_output = metatree_eval_metric.compute()

    logger.info(f"epoch {epoch}: train_acc: {train_output['accuracy']} train_loss: {(total_loss):.4f} eval_acc: {eval_output['accuracy']} num_of_trees: {len(decision_tree_forest.trees)}")
