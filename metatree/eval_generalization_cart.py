...
def main():
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers("treegen_stats_cart", experiment_config)
    ...
    while len(decision_tree_forest) < args.max_train_steps:
        for step, batch in enumerate(train_dataloader):
            ...
            decision_tree_forest.append(DecisionTreeClassifier(random_state=0, max_depth=args.depth).fit(X, Y))
            assert decision_tree_forest[-1].tree_.node_count <= 2**(args.depth+1) - 1
            y_pred = decision_tree_forest[-1].predict(X)
            train_acc.append(accuracy_score(Y, y_pred))
            if len(decision_tree_forest) >= (args.max_train_steps-1):
    ...
    for step, batch in enumerate(train_dataloader):
        with torch.no_grad():
            ...
            for tree_model in decision_tree_forest:
                tree_pred = tree_model.predict(batch["input_x"].detach().cpu().numpy())
    ...
    logger.info(f"epoch {epoch}: train_acc: {torch.tensor(train_acc).mean()} eval_acc: {eval_output['accuracy']} num_of_trees: {len(decision_tree_forest)}")
